...
This operator does not bring anything breaking new, it just offers 
...
The \verb'given $value' statement ``topicalises'' the argument, 
...
pairs using the colon-pair syntax. In the signature, 
...
if you want to compare your solutions and results with ours. 
...
the \verb'<orange banana>' list in the last example above. 
...
the set operators commonly used in mathematics, we've only shown 
...
study our solution: \ref{sol_markov_analysis}.
...
to build stacks and queues. It was fairly easy. In some lower-
level programming languages, you would need linked lists for that.
\index{stack}
\index{queue}
...
linked list. There is no need to use anything like 
...
represents the letters SOS, which comprise an internationally-
recognized call for help. The table in Figure~\ref{fig.morse} 
shows the rest of the codes.
\index{Morse code}
...
\index{The Gold-Bug (Edgar Allen Poe)}
\index{cypher}
