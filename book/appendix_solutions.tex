...
\subsection{Exercise~\ref{draw_a_grid}: subroutine print-grid (p.~\pageref{draw_a_grid})}
...
For asking a user to enter a number of seconds, you might 
do this:
\index{prompt}
...
(\ref{sol_fermat_ex}) gives an example of a recursive 
...
\subsection{Exercise~\ref{fermat_ex}: Fermat's theorem (page~\pageref{fermat_ex})}
\label{sol_fermat_ex}
\index{Fermat, Pierre}
...
we haven't covered yet, \emph{multi} subroutines (see 
section~\ref{multisubs}, p.~\pageref{multisubs}, for details). 
...
\subsection{Compare (at the end of section~\ref{compare}, p.~\pageref{compare})}
\label{sol_compare}
...
\subsection{Hypotenuse (at the end of section~\ref{hypotenuse}, p.~\pageref{hypotenuse})}
...
The first idea that may come to mind is to build a modified 
...
As it can be seen, this is quite complicated because of 
...
We will see later ways to better organize test cases (see 
for example \ref{test_module}, p. \pageref{test_module}).
...
example code for stacks (see section~\ref{stack_code} 
on page~\pageref{stack_code}) the changes we have made 
...
chapter, please make sure to read the 
\ref{array_functional_programming} subsection in the 
...
Finding reverse pairs requires to read each word of 
...
multidimensional arrays (see \ref{multidimensional_array}. 
...
(see \ref{sol_has_duplicates}) and ``birthday paradox'' (see 
\ref{sol_birthdays}) exercises to get some coding clues. An 
...
in section~\ref{test_module} (page~\pageref{test_module}).
...
\subsection{Exercises on the Huffman Code (section~\ref{huffman_exercise}, \pageref{huffman_exercise})}
...
This grammar and its associated actions class may be 
