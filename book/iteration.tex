...
within a subroutine (with the {\tt my} keyword or \emph{declarator}
\footnote{A declarator is just a Perl keyword used for 
declaring a new variable or other identifier. The most 
common declarator is \{tt my}, which declares a lexical 
variable. Others include {\tt our} or {\tt state}, nbriefly 
mentioned later in this chapter.}) 
are \emph{local} to that subroutine. Similarly, subroutine 
...
might not converge as fast and as neatly as Newton's algorithm but might instead alternate values above and below the accurate result.
