...
and  gives a value to a variable, but you first need to 
...
the second gives the integer {\tt 42} to {\tt \$number}; the 
third reassigns the integer 17 to {\tt \$number}; the fourth 
assigns the (approximate) value of the golden ratio to {\tt \$phi}.

There are two important syntax features to explain here.

First, in Perl, variable names start with a so-called \emph{sigil}, i.e. a special non-alphanumeric character such 
as \$, \@, \%, \& and some others. This special character 
...
means that the declaration has to be before (or at the point of) its 
first use in the text file containing the program. We will see later that 
programs don't necessarily run from top to bottom in the order in which 
the program lines appear in the program file; still, the variable 
declaration must be before its use in the text file containing the program.

If you omit to declare a variable, you get a syntax error:
...
{\tt \$brücke}, {\tt \$payé} or {\tt \$niño} are valid variable names, which
can be useful for non-English programmers (provided that 
these Unicode characters are handled correctly by your text editor and your 
screen configuration). Similarly, instead of using 
\verb"$phi" for the name of the golden ratio variable, 
we might have used the \emph{Greek small letter phi}, $φ$ 
(Unicode code point U+03C6), just as we could have used 
the \emph{Greek small letter pi}, $π$,  for the well-known 
...
The underscore character, \verb"_", can appear anywhere in a name.
It is often used in names with multiple words, such as 
\verb"$your_name" or \verb"$airspeed_of_unladen_swallow". 
\index{underscore character}

You may even use so-called ``kebab case''\footnote{Because the 
...
brevity, we will omit the print statements in the 
...
that Perl, like many other languages, has a shortcut for this:
...
such as the string concatenation operator, but we will see that later.
...
end with {\tt .pl} or {\tt .pl6}.
\index{script}
\index{script mode}

Please make sure that you're really using a \emph{text editor} 
and not a \emph{word processing software} (such as MS Word, 
...
capabilities, which might help you use a correct syntax (and 
...
saved in a file but typed at the prompt between quote marks immediately after 
this option.
...
later in this book (see for example 
subsection~\ref{one-liner example}, subsection~\ref{rot13_oneliner} solving the ``rot-13'' exercise or
...
\item Operators with the same precedence are usually evaluated from left to
  ...
  constant in Perl~6, and therefore does not require a sigil).  To divide by $2 \pi$, you can use parentheses or write
  {\tt \$degrees / 2 / pi} or {\tt \$degrees / } $2 / \pi$.
...
the strings look so much like numbers that they can be transformed or \emph{coerced} into 
numbers and still make sense, so the following are illegal:
...
{\tt 8)} is a {\bf syntax error}. \footnote{We are using here 
...
of section~\ref{string_operations} (page~\pageref{string_operations}), 
...
\item[Semantic error:] The third type of error is ``semantic'', which
  ...
  it will do what you told it to do, but not what you intended it to do.
...
\item How about {\tt \$x = \$y = 1} (hint: note that you will have to declare
both variables, for example with a statement such as {\tt my \$x; my \$y);} 
or possibly {\tt my (\$x, \$y);}, 
